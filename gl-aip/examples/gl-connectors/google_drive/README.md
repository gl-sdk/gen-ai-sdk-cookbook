...
This folder contains example scripts demonstrating the Google Drive connector with the GL Connectors SDK.
...
Run `uv sync` from inside the `google_drive` folder to install the project's dependencies (reads `pyproject.toml`):

```bash
# from the google_drive folder
uv sync
```
...
Run these `uv` commands from inside the `google_drive`  folder.

```bash
uv run examples_draft_collector.py
uv run examples_get_current_file_and_change_permission.py
uv run examples_move_file_to_new_folder.py
```

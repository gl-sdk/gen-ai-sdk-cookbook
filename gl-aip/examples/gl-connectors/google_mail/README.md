# Google Mail Connector Examples

---

## Overview

This folder contains example scripts demonstrating the Google Mail connector with the GL Connectors SDK.
...
Run `uv sync` from inside the `google_mail` folder to install the project's dependencies (reads `pyproject.toml`):

```bash
# from the google_mail folder
uv sync
```
...
Run these `uv` commands from inside the `google_mail`  folder.

```bash
uv run examples_send_mail.py
```
